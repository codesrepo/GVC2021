...
if __name__ == '__main__':
    ...
    for metric in param_fairness['protected_attribute_list']:     
        new_obj = FairnessMaster(final,param_fairness['target'],param_ROC['score'],metric,param_fairness['privileged_group'],param_fairness['base_trheshold'],param_fairness['threshold_bins'])
    ...
    for metric in metric_summary_paths:
        ...
        line_plot(df_metrices,'Score','Fairness values','Fairness report- Reject option classifier(%s)'%(metric),['threshold'],["threshold"]+param_fairness['track_metrices'],'threshold','value',vertical=0.93,save_path=save_path)
